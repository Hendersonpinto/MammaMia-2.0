class DashboardsController < ApplicationController
    ...
    @my_past_bookings = Booking.where(user: current_user).where('start_date < ?', Date.today)
    @my_current_bookings = Booking.where(user: current_user).where('start_date >= ?', Date.today)
